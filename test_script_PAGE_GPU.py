...
edge angle corresponding to color. This script supports CPU or GPU computation.
Default is GPU.
...
image_index = 1  # Index of image in list to create filepath and run, may be looped over for many images.
...
# Convert image to torch tensor for calculations
Image_grey_double = torch.tensor(Image_orig, dtype=tfloat, device=device)
...
if device == "cpu":
   ...
   # return the output from torch tensor on the GPU to numpy on the CPU
